...
function applySplices(orig, copy) {
  ...
  if (summaries && summaries.length &&
    ...
    splices.forEach(function(splice) {
      ...
      console.log("splice.apply(" + JSON.stringify(copy) + ", " + JSON.stringify(spliceArgs) + ")");
...
function randomArrayOperation(arr) {
  ...
  if (operation == 'delete') {
    ...
    console.log("deleting index " + index + " from " + JSON.stringify(arr));
    delete arr[index];
    console.log("    resulted in: " + JSON.stringify(arr));
  } else if (operation == 'update') {
    var index = randInt(0, arr.length - 1);
    var newval = randInt(0, valMax);
    console.log("updating index " + index + " to " + newval + " in " + JSON.stringify(arr));
    arr[index] = newval);
    console.log("    resulted in: " + JSON.stringify(arr));
  } else {
    var opArgs = operations[operation]();
    var func = arr[operation];
    console.log("applying " + operation + "(" + JSON.stringify(arr) + ", " + JSON.stringify(opArgs) + ")");
